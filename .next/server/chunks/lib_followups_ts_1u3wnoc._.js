module.exports=[94689,e=>{"use strict";var t=e.i(54799),a=e.i(85148),r=e.i(14747),i=e.i(22734);let o=process.env.DATA_DIR??r.default.join(process.cwd(),"data"),s=r.default.join(o,"daisy.db"),n=[r.default.join(o,"products-backup.json"),r.default.join(process.cwd(),"data","products.json")],c=null;function l(){return c||((0,i.existsSync)(o)||(0,i.mkdirSync)(o,{recursive:!0}),(c=new a.default(s)).pragma("journal_mode = WAL"),c.pragma("foreign_keys = ON"),function(e){e.exec(`
  ...
  `);try{e.exec("ALTER TABLE orders ADD COLUMN bank_id TEXT")}catch{}try{e.exec("ALTER TABLE orders ADD COLUMN tracking_number TEXT")}catch{}try{e.exec("ALTER TABLE installment_applications ADD COLUMN product_imageUrl TEXT")}catch{}try{e.exec("ALTER TABLE installment_applications ADD COLUMN quantity INTEGER NOT NULL DEFAULT 1")}catch{}}(c),function(e){try{e.exec("ALTER TABLE products ADD COLUMN originalPrice TEXT NOT NULL DEFAULT ''")}catch{}}(c),function(e){if(!e.prepare("SELECT name FROM migrations WHERE name = ?").get("json_import_gadgets_v1")){for(let t of n)if((0,i.existsSync)(t))try{let a=JSON.parse((0,i.readFileSync)(t,"utf-8"));if(Array.isArray(a)&&a.length>0){e.prepare("DELETE FROM products").run();let t=e.prepare(`
...
</html>`}(l),attachments:c.length?c:void 0,headers:{"List-Unsubscribe":`<${r}>`,"List-Unsubscribe-Post":"List-Unsubscribe=One-Click"}})}process.env.BANK_NAME,process.env.BANK_ACCOUNT_HOLDER,process.env.BANK_ACCOUNT_TYPE,process.env.BANK_ACCOUNT_NUMBER,process.env.BANK_BRANCH_CODE;let w="https://gadgets.bevanssons.store";function X(e,t,a,r){if(r)return!!e.prepare("SELECT id FROM email_sends WHERE LOWER(email) = ? AND type = ? AND ref = ?").get(t.toLowerCase(),a,r);let i=new Date(Date.now()-3024e6).toISOString();return!!e.prepare("SELECT id FROM email_sends WHERE LOWER(email) = ? AND type = ? AND createdAt > ?").get(t.toLowerCase(),a,i)}function D(e,t){e.prepare(`
    INSERT OR IGNORE INTO email_sends (id, email, type, ref, subject, createdAt)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(t.id,t.email.toLowerCase(),t.type,t.ref??null,t.subject,new Date().toISOString())}async function I(){let e=l(),a=0,r=0;for(let i of e.prepare(`
...
You browsed some great products but didn't complete your order. Your items are still available — grab them before they sell out!`,ctaText:"Complete Your Order",ctaUrl:d,orderItems:s,restoreCartUrl:d,trackingId:n}),D(e,{id:n,email:i.email,type:"cart_abandon_1d",ref:o,subject:l}),a++}catch(e){console.error("[followups] cart_abandon_1d error:",e)}}for(let i of e.prepare(`
...
We'd love to hear your feedback — it takes less than a minute and helps us serve you better.`,ctaText:"Leave a Review",ctaUrl:`${w}/reviews`,trackingId:o}),D(e,{id:o,email:i.email,type:"delivery_followup",ref:i.ref,subject:n}),a++}catch(e){console.error("[followups] delivery_followup error:",e)}}for(let i of e.prepare(`
...
We have amazing new arrivals and deals that we think you'll love. Come back and see what's new!`,ctaText:"Shop New Arrivals",ctaUrl:`${w}/new-arrivals`,trackingId:o}),D(e,{id:o,email:i.email,type:"reengagement_30d",subject:n}),a++}catch(e){console.error("[followups] reengagement_30d error:",e)}}return console.log(`[followups] sent=${a} skipped=${r}`),{sent:a,skipped:r}}e.s(["runFollowUps",0,I],94689)}];
