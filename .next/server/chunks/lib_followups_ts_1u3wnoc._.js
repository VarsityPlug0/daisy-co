...
<body style="margin:0;padding:0;background:${g};font-family:'Helvetica Neue',Helvetica,Arial,sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:${g};padding:28px 16px">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:16px;overflow:hidden;border:1px solid ${u}">
        ...
        <tr><td style="background:linear-gradient(90deg,${g},${T},${m},${T},${g});height:3px;font-size:0;line-height:0">&nbsp;</td></tr>
          ...
          <td style="background:${g};padding:24px 36px">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td align="right">
                  <a href="${L}" style="color:${E};font-size:12px;text-decoration:none">gadgets.bevanssons.store</a>
                </td>
              </tr>
            </table>
          </td>
        ...
        <tr>
          <td style="background:${g};padding:24px 36px;border-top:1px solid ${u}">
            <p style="margin:0 0 8px;color:${E};font-size:12px;text-align:center">
              Questions? &nbsp;
              <a href="mailto:support@bevanssons.store" style="color:${T};text-decoration:none;font-weight:600">support@bevanssons.store</a>
              &nbsp;\xb7&nbsp;
              <a href="${L}" style="color:${T};text-decoration:none;font-weight:600">gadgets.bevanssons.store</a>
            </p>
        ...
        <!-- Bottom gold bar -->
        <tr><td style="background:linear-gradient(90deg,${g},${T},${m},${T},${g});height:2px;font-size:0;line-height:0">&nbsp;</td></tr>

      </table>
    </td></tr>
  </table>
</body>
</html>`}(s),attachments:o.length?o:void 0})}let R="https://gadgets.bevanssons.store";function b(e,t,a,i){if(i)return!!e.prepare("SELECT id FROM email_sends WHERE LOWER(email) = ? AND type = ? AND ref = ?").get(t.toLowerCase(),a,i);let r=new Date(Date.now()-3024e6).toISOString();return!!e.prepare("SELECT id FROM email_sends WHERE LOWER(email) = ? AND type = ? AND createdAt > ?").get(t.toLowerCase(),a,r)}function O(e,t){e.prepare(`
  ...
  `).all()){let i=a.lastAdded.slice(0,10);if(b(e,a.email,"cart_abandon_1d",i)){d++;continue}let r=e.prepare(`
      ...
      LIMIT 6
    `).all(a.email.toLowerCase()),o=(0,t.randomUUID)(),s=(a.name??"there").split(" ")[0],n=`${s}, you left something behind — Bevanssons`,c=Buffer.from(JSON.stringify(r)).toString("base64"),l=`${R}/restore-cart?items=${c}`;try{await S({to:a.email,name:a.name??"there",subject:n,heading:"Your cart is waiting for you",body:`Hi ${s},

You browsed some great products but didn't complete your order. Your items are still available — grab them before they sell out!`,ctaText:"Complete Your Order",ctaUrl:l,orderItems:r,restoreCartUrl:l,trackingId:o}),O(e,{id:o,email:a.email,type:"cart_abandon_1d",ref:i,subject:n}),p++}catch(e){console.error("[followups] cart_abandon_1d error:",e)}}for(let a of e.prepare(`
    SELECT id, ref, email, name FROM orders
    WHERE status = 'delivered'
      AND updatedAt < datetime('now', '-3 days')
      AND updatedAt > datetime('now', '-14 days')
  `).all()){if(b(e,a.email,"delivery_followup",a.ref)){d++;continue}let i=(0,t.randomUUID)(),r=a.name.split(" ")[0],o=`How was your order, ${r}? — Bevanssons`;try{await S({to:a.email,name:a.name,subject:o,heading:"How was your experience?",body:`Hi ${r},

Your order ${a.ref} was delivered recently and we hope you're loving it! 🎉

We'd love to hear your feedback — it takes less than a minute and helps us serve you better.`,ctaText:"Leave a Review",ctaUrl:`${R}/reviews`,trackingId:i}),O(e,{id:i,email:a.email,type:"delivery_followup",ref:a.ref,subject:o}),p++}catch(e){console.error("[followups] delivery_followup error:",e)}}for(let a of e.prepare(`
  ...
  `).all()){if(b(e,a.email,"reengagement_30d")){d++;continue}let i=(0,t.randomUUID)(),r=a.name.split(" ")[0],o=`We miss you, ${r}! — Bevanssons`;try{await S({to:a.email,name:a.name,subject:o,heading:`We miss you, ${r}!`,body:`Hi ${r},

It's been a while since your last order and we wanted to check in.

We have amazing new arrivals and deals that we think you'll love. Come back and see what's new!`,ctaText:"Shop New Arrivals",ctaUrl:`${R}/new-arrivals`,trackingId:i}),O(e,{id:i,email:a.email,type:"reengagement_30d",subject:o}),p++}catch(e){console.error("[followups] reengagement_30d error:",e)}}return console.log(`[followups] sent=${p} skipped=${d}`),{sent:p,skipped:d}}e.s(["runFollowUps",0,w],94689)}];
