...
export const metadata: Metadata = {
  title: "About Us | Bevanssons — Premium Gadgets Worldwide",
  description: "Bevanssons is a proudly South African gadget retailer offering premium electronics, appliances, gaming, solar & more with worldwide shipping.",
};
    ...
    <div className="max-w-5xl mx-auto px-5 sm:px-8 py-20">
      ...
      <div className="mb-16 text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-6">
          About <span className="gold-text">Bevanssons</span>
        </h1>
      ...
      {/* Mission */}
      <div className="bg-[#1D1D1D] border border-[#C8B993]/20 rounded-3xl p-10 mb-12 text-center">
        <p className="text-[#C8B993] text-xs uppercase tracking-widest font-bold mb-4">Our Mission</p>
      ...
      <div className="grid md:grid-cols-2 gap-10 mb-16">
        ...
        <div className="bg-[#1D1D1D] border border-[#2A2A2A] rounded-2xl p-7">
          <h3 className="text-white font-bold text-lg mb-5">Find Us</h3>
          <div className="space-y-4 text-sm">
            <div className="flex items-start gap-3">
              <MapPin size={16} color="#C8B993" className="shrink-0 mt-0.5" />
              <div>
                <p className="text-white font-medium">36 Houer Road</p>
                <p className="text-gray-500">City Deep, Johannesburg, 2197</p>
                <p className="text-gray-500">South Africa</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <Mail size={16} color="#C8B993" className="shrink-0" />
              <a href="mailto:support@bevanssons.store" className="text-white hover:text-[#C8B993] transition-colors">support@bevanssons.store</a>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {values.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="bg-[#1D1D1D] border border-[#2A2A2A] rounded-2xl p-6 text-center">
              <div className="w-11 h-11 rounded-xl bg-[#C8B993]/10 flex items-center justify-center mx-auto mb-3">
      ...
      {/* Coverage */}
      <div className="bg-[#1D1D1D] border border-[#2A2A2A] rounded-2xl p-8 mb-12">
        ...
        <div className="flex flex-wrap gap-2">
          {areas.map((a) => (
            <span key={a} className={`text-xs font-semibold px-3 py-1.5 rounded-full border ${
              a.includes("International") ? "border-[#C8B993]/40 text-[#C8B993] bg-[#C8B993]/5" : "border-[#2a2a2a] text-gray-400"
            }`}>{a}</span>
          ))}
        </div>
      </div>
