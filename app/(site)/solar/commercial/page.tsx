...
export default function CommercialSolar() {
  ...
  return (
    <>
      {/* ── HERO ── */}
      <section className="relative bg-[#111111] overflow-hidden" style={{ minHeight: 300 }}>
        <Image
        ...
        <div className="absolute inset-0"
          style={{ background: "radial-gradient(ellipse at 80% 50%, rgba(200,185,147,0.1) 0%, transparent 60%)" }} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-10 lg:px-16 py-12 flex flex-col items-start justify-center" style={{ minHeight: 300 }}>
          <Link href="/solar" className="text-[#C8B993] text-sm hover:underline mb-4 inline-flex items-center gap-1.5 opacity-80 hover:opacity-100 transition-opacity">
            &larr; Solar Solutions
          </Link>
          <p className="text-[#C8B993] text-[11px] uppercase tracking-widest font-semibold mb-3">For Your Business</p>
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white leading-tight mb-3">
            Commercial{" "}
            <span className="text-[#C8B993]">Solar Systems</span>
          </h1>
      ...
      {/* ── ROI STATS ── */}
      <section className="bg-[#111111] py-8 border-b border-[#1A1A1A]">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {benefits.map((b) => (
              <div key={b.label} className="bg-[#1D1D1D] border border-[#2A2A2A] rounded-2xl p-6 text-center card-hover">
                <p className="text-3xl md:text-4xl font-extrabold text-[#C8B993] mb-2">{b.value}</p>
      ...
      <section className="bg-[#111111] py-12">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
          <div className="text-center mb-8">
            ...
            <div className="w-12 h-0.5 bg-[#C8B993] mx-auto mb-4" />
            <p className="text-gray-400 text-base max-w-lg mx-auto">Commercial solar solutions for every type of business.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {segments.map((seg) => (
              <div key={seg.name} className="bg-[#1D1D1D] border border-[#1A1A1A] rounded-2xl overflow-hidden card-hover flex flex-col group">
                <div className="relative h-52 overflow-hidden">
                  <Image src={seg.img} alt={seg.name} fill
                    className="object-cover transition-transform duration-500 group-hover:scale-105"
                    sizes="(max-width: 640px) 100vw, 50vw" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#1D1D1D]/90 via-black/30 to-transparent" />
      ...
      <section className="bg-[#0f0f0f] py-12 border-t border-[#1A1A1A]">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
          <div className="bg-[#1D1D1D] border border-[#C8B993]/20 rounded-2xl p-6 sm:p-8">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-bold text-white mb-3">Why Go Solar for Your Business?</h2>
              <div className="w-12 h-0.5 bg-[#C8B993] mx-auto" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {reasons.map((r) => (
                <div key={r.text} className="flex items-start gap-4">
                  <div className="w-8 h-8 rounded-full bg-[#C8B993]/10 border border-[#C8B993]/30 flex items-center justify-center shrink-0 mt-0.5">
                    <r.icon size={15} color="#C8B993" strokeWidth={2} />
                  </div>
      ...
      {/* ── CTA ── */}
      <section className="bg-[#111111] py-12 border-t border-[#1A1A1A]">
