  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-8 py-12">
      ...
      <div className="text-center mb-10">
        <p className="text-[#C8B993] text-sm uppercase tracking-widest mb-3">Gadgets & Gear</p>
        ...
        {categories.map((cat) => (
          <div key={cat.name}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {cat.items.map((item) => (
                <div key={item.name} className="bg-[#1D1D1D] border border-[#2A2A2A] rounded-2xl overflow-hidden card-hover flex flex-col group">
                  {/* Image */}
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    <p className="text-xs text-[#C8B993] uppercase tracking-wider mb-1.5">{cat.name}</p>
                    <p className="font-semibold text-white text-sm leading-snug mb-2 flex-1">{item.name}</p>
                    <div className="flex items-center gap-2 mb-3">
                      <p className="text-[#C8B993] font-bold text-lg">{item.price}</p>
